...
#     wb_calc_good, # pyright: ignore
#     wb_profit_calc_good, # pyright: ignore
#     wb_bulk_calc_good, # pyright: ignore
#     wb_log_good, # pyright: ignore
#     wb_returns_good, # pyright: ignore
# )

client = TestClient(app)


def test_wb_price_calc(wb_calc_good):
    response = client.post("/api/v1/wb/prices/calculate", json=wb_calc_good)
    assert response.status_code == 200


def test_wb_profit_calc(wb_profit_calc_good):
    response = client.post(
        "/api/v1/wb/profits/calculate",
        json=wb_profit_calc_good,
    )
    assert response.status_code == 200
