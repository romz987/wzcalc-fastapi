...
| Обозначение | Параметр            | Описание                        |
|-------------|---------------------|---------------------------------|
| $C$         | count               | Количество товаров в упаковке   |
| $C_{p}$     | cost_per_one        | Себестоимость единицы товара    |
| $C_{v}$     | cost_value          | Себестоимость товара            |
| $K_{p}$     | comission_percent   | Процент комиссии маркетплейса   |
| $K$         | comission_fee       | Стоимость комиссии в рублях     |
| $M_{p}$     | last_mile_percent   | Процент "последней мили"        |
| $M$         | last_mile_fee       | Стоимость "последней мили"      |
| $Q_{p}$     | acquiring_percent   | Процент эквайринга маркетплейса |
| $Q$         | acquiring_fee       | Стоимость эквайринга в рублях   |
| $T_{p}$     | tax_percent         | Процент налога                  |
| $T$         | tax_fee             | Стоимость налога в рублях       |
| $Rs_{p}$    | risk_percent        | Процент рисков                  |
| $Rs$        | risk_fee            | Стоимость рисков в рублях       |
| $B$         | box_cost            | Стоимость упаковки              |
| $W$         | wage_cost           | Стоимость труда                 |
| $L$         | logistics_fee       | Стоимость логистики             |
...
- Идея расчета:
  ...
  Профитом считается разность между ценой продажи товара на маркетплейсе и всеми расходами, которые несет продавец.  
  Сначала рассчитывается себестоимость товара: для этого цена за единицу товара умножается на количество товара в упаковке.  
  Затем, рассчитывается стоимость последней мили в зависимости от установленного маркетплейсом процента последней мили.  
  Стоимость последней мили не может превышать 500 рублей - если расчетная стоимость выше, то значение last_mile_fee все равно остается 500 рублей.  
  Далее, рассчитываются стоимость комиссии маркетплейса и эквайринга отталкиваясь от установленных маркетплейсом процентов.  
  Следом, при "простой" системе налогооблажения рассчитываются стоимость налога и риски, как соотвествующий процент от цены товара на маркетплейсе.  
  Все расходы расчитываются как сумма следующих состовляющих:  
    ...
    - стоимость логистики  
    - стоимость возвратов  
  Далее, все расходы вычитаются из цены товара на маркетрлейсе.
...
$A_{c} = C_{v} + K + M + Q + T + Rs + B + W + L + Rt + Sh$
...
Система налогооблажения simple подразумевает, что налог будет расчитан от стоимости товара.
...
- Идея расчета:
  ...
  Профитом считается разность между ценой продажи товара на маркетплейсе и всеми расходами, которые несет продавец.  
  Сначала расчитывается себестоимость товара: для этого цена за единицу товара умножается на количество товара в упаковке.  
  Далее, расчитываются стоимость комиссии маркетплейса и эквайринга отталкиваясь от установленных маркетплейсом процентов.  
  Следом, при "простой" системе налогооблажения расчитываются стоимость налога и риски, как соотвествующий процент от цены товара на маркетплейсе.  
  Все расходы расчитываются как сумма следующих состовляющих:  
    - cебестоимость товара  
    - стоимость комиссии  
    - стоимость эквайринга  
